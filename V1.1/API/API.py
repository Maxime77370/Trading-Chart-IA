...
con = fxcmpy.fxcmpy(access_token='f76580f554aaacd1dac424d680c3ce2d117baad5', log_level='error', server='demo', log_file='error')
print("Log in complete")

try:	# on essaye de crée un ficher si celui-ci est deja crée alors ne rien faire
	path = os.getcwd().replace("/API","") + str("/DATA") #créeation de la direction du fichier.
	os.mkdir(path)
...
for x in range(len(instruments)):
	...
	fullpath = path + str("/") + str(instruments[x]).replace("/", "-") + ".csv"
